#################################################################
#
# Project name: temporal and thermal effects on the abundance of lethal 
...
nameOfile= 'Desktop/LA_Isolate_%d'
str(tempOisolate)

#import in the excel file
Isolate_Data <- read_excel("Downloads/Summer Research 2024/LA.Isolate_.xlsx")
...
for (q in 1:length(tempOisolate)) {
  ...
  Isolate = grep('LA',Isolate_Data$Isolate)
  ...
  result0 = optim(c(KAguess, X0Aguess,RAguess), errlogfun , method = "L-BFGS-B",
                  ...
                  upper = c(510, 0.5, 0.9),MTTdata=MTTdata,tid=tid)
  ...
  #result3 = optim(c(X0Dguess, R0Dguess,R1Dguess,W1Dguess,W2Dguess,K1Dguess,K0Dguess),
  #                errgrowthcarfun, method = "L-BFGS-B",
  #                lower = c(0.1*X0Dguess, 0.06*R0Dguess, max(-0.1*R1Dguess,-1), 
  #                          0.5*W1Dguess, -W2Dguess,
  #                          -0.02*K1Dguess, 0.01*K0Dguess),
  #                upper = c(1.2*X0Dguess,5*R0Dguess, min(8*R1Dguess,1),
  #                          4.5*W1Dguess,7*W2Dguess,1*K1Dguess,2.5*K0Dguess),
  #                MTTdata=MTTdata,tid=tid) 
  #calculate the result of the optimized parameters
  result3$par
  result3$value
